...
test('should display a modal with when isAddingNewLocation', () => {
  ...
  let wrapper = shallow(<LocationList locations={[]} isAddingNewLocation={false} />);

  // No modal shows when not adding location
  expect(wrapper.find(Modal).length).toBe(0);

  wrapper = mount(<LocationList locations={[]} isAddingNewLocation={true} onHideLocationCreator={mockOnHideLocationCreator} />);
...
test('should call onShowLocationCreator when button is clicked', () => {
  ...
  const wrapper = shallow(<LocationList locations={[]} onShowLocationCreator={mockProp} />);
